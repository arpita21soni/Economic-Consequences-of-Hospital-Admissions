...
- It runs our main parameteric event study for the HRS, which we use to calculate implied
  effects based on the timing for each of the variables. This program includes additional controls
  relative to our primary specification to test robustness.
*/
...
matrix define results_IEs = J(19,`J',.)

local j = 1
foreach v of varlist `outcomes' { 

	local controls "age_poly* female black hispanic race_other hs_grad some_college collegeplus"
	...
	* Constant term for adjustment:
	predict yhat, xb
	sum yhat [aweight=rwtresp] if evt_time==-1
		matrix results_IEs[19,`j'] = r(mean)
	drop yhat
...
local r="b_12mEarn se_12mEarn b_36mEarn se_36mEarn b_36aEarn se_36aEarn b_12mOOP se_12mOOP b_36mOOP se_36mOOP b_36aOOP se_36aOOP b_12mLFP se_12mLFP b_36mLFP se_36mLFP b_36aLFP se_36aLFP yhat_f1"
...
foreach type of local types {
	 ...
	 outsheet using "`output'/HRS_EScontrols_`type'_`samp'_`spouse'_`spec'_FE`fe'.txt", replace
...
order var
outsheet using "`output'/HRS_IEscontrols_`samp'_`spouse'_`spec'_FE`fe'.txt", replace
