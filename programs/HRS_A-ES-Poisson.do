...
- It runs our Poisson event study for the HRS, which we use to calculate implied
...
matrix define results_IEs = J(18,`J',.)
...
foreach v of local outcomes { 
	...
	* Keep only those with non-missing observations
	drop if missing(`v')
	...
	* Drop if in AHEAD or CODA cohorts for under65 because tiny number causing collinearity
	if ("`samp'"=="under65_INS"|"`samp'"=="under60_INS") {
		drop if inlist(hacohort,1,2)
	}
	egen cohortXwave = group(hacohort wave)
	xi i.`fe' 
	
	di "FE = `fe', Sample = `samp', Spec = `spec', Var = `v', Spouse = `spouse'"
	poisson `v' evt_time evt_l* _IcohortXwa* [pweight=rwtresp], cluster(hhidpn) iter(150)
	...
	* LFP
	lincom 1.627*evt_l0 - 0.293*evt_l1 + 0.0388*evt_l2
		matrix results_IEs[13,`j'] = r(estimate)
		matrix results_IEs[14,`j'] = r(se)
	lincom -0.248*evt_l0 + 1.373*evt_l1 - 0.182*evt_l2
...
local r="b_12mEarn se_12mEarn b_36mEarn se_36mEarn b_36aEarn se_36aEarn b_12mOOP se_12mOOP b_36mOOP se_36mOOP b_36aOOP se_36aOOP b_12mLFP se_12mLFP b_36mLFP se_36mLFP b_36aLFP se_36aLFP"
...
foreach type of local types {
	 ...
	 outsheet using "`output'/HRS_Poisson_`type'_`samp'_`spouse'_`spec'_FE`fe'.txt", replace
...
order var
outsheet using "`output'/HRS_Poisson_IEs_`samp'_`spouse'_`spec'_FE`fe'.txt", replace 
